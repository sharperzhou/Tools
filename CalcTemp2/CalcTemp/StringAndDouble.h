...
///////////////////////////////////////////////////////
/*
功能：用于C字符串与数值转换，包括字符串转换为double，
	  double转换为完整的字符串，double转格式化的字
	  符串。

自定义变量类型：1、枚举变量Type，用于设定格式化字符串
		        的类型，固定长度指小数位数根据数值大小
			    可变，固定小数位数根据四舍五入法。
			    2、结构体类型FormatType，用于指定固定
			    长度及保留的小数位数。

公开成员函数：1、StrtoD，实现字符串转double，返回值标示
			  转换成功与否（下同），输出结果为引用类型。
			  2、DtoFormatStr，实现double转成指定格式的
			  字符串，输出结果为char *指针。
			  3、DtoWholeStr，double转成完整的字符串，
			  与编译器一致，最多保留18位长度，可根据数
			  值大小自动转换为科学计数。

私有成员函数：1、FormatFixedLengthStr，固定长度格式化的
              处理子函数，无返回值。
			  2、formatStr，保留小数位数的格式化字符串，
			  用于sprintf函数，返回值为char *。

公开数据成员：无。

私有数据成员：_strfmt[6]，保留小数位数的格式化字符串，
              用保存格式化字符串内容。
*/
///////////////////////////////////////////////////////
